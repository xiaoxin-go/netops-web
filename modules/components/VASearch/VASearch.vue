  ...
  <span class="va-search">
    <span class="search-item" v-for="item in options" :key="item.field">
      ...
      <template v-else-if="item.tagType in components">
        <component :is="components[item.tagType]"
    ...
    <a-button type="primary" style="margin-left: 5px" :size="size" @click="onSearch">搜索</a-button>
  </span>
</template>
...
const parseValue = () => {
  ...
  const _push = (item: VASearchItem) =>{
    console.log(source.value, item.field)
    if (item.paramsType === 'query')        // 如果查询类型是query，则需要添加到query查询内
      result.query.push({
        key: item.field,
      ...
      result[item.field] = source.value[item.field]     // 否则直接赋值即可
  }
  // 循环查询项，查询时，把查询项根据查询类型插入到查询参数中
  props.options.map((item: VASearchItem) => {
    if (!(item.field in source.value) || source.value[item.field] === undefined)     // 此处判断，是为搜索框值为空，则从params中去除
      return
    _push(item)
    // 级联的也添加
  ...
  console.log("search-params:",result)
  return result
}
