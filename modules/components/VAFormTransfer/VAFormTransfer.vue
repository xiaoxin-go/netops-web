<template>
  <a-drawer :visible="visible"
            ...
            width="800px"
            class="custom-class"
            :title="`关联角色租户-${props.transfer.title}`"
            placement="right">
    <!-- :titles="['未被分享的用户', '已被分享的用户']" -->
    <a-transfer v-model:target-keys="data.targetList"
                :data-source="data.dataSource"
                :render="item => item.title"
                :show-search="props.transfer.showSearch?props.transfer.showSearch:true"
                :pagination="props.transfer.pagination ? { pageSize:props.transfer.pageSize }:false"
                style="height:100%"
                @search="search" />
    ...
    <template #footer>
      <slot name="action">
        <a-button style="margin-right: 8px"
                  size="small"
                  @click.prevent="onCancel">Cancel</a-button>
