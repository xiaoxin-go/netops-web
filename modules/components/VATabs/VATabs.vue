<template>
  <a-tabs :activeKey="value"
          :tab-position="options.position"
          :type="options.type"
          :tab-bar-gutter="options.barGutter"
          :size="options.size"
          @change="onChange">
    <a-tab-pane v-for="item in source" :key="item.value" :disabled="item.disabled">
      <template #tab>
        <span>
          {{item.label}}
        </span>
...
import {defineEmits, defineProps, PropType, watch, onMounted, computed} from "vue";
