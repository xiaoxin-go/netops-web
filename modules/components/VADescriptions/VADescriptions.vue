<template>
  <a-descriptions :title="title" :column="column">
    <template #extra>
      <slot name="extra">
      </slot>
    </template>
    <a-descriptions-item v-for="item in options" :key="item.label" :label="item.label" :span="item.span || 1">
      <template v-if="item.slot && item.slot !== ''">
        <slot :name="item.slot"></slot>
      </template>
      <a v-else-if="item.tagType === 'a'" :style="item.style">{{ data[item.key] }}</a>
      <a-tag v-else-if="item.tagType === 'tag'" :color="item.colors[data[item.key]]">
